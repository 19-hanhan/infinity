...
public:
    ...
    UniquePtr<IndexHnsw> MakeIndexHnsw() {
        return MakeUnique<IndexHnsw>(index_name, nullptr, filename, column_names, metric_type,
                                        encode_type, build_type, M, ef_construction, chunk_size, None);
    }

    UniquePtr<IndexHnsw> CompressMakeIndexHnsw() {
        return MakeUnique<IndexHnsw>(index_name, nullptr, filename, column_names, metric_type,
                                        HnswEncodeType::kLVQ, build_type, M, ef_construction, chunk_size, None);
    }
...
TEST_F(HnswHandlerTest, test_save_load) {
...
TEST_F(HnswHandlerTest, test_compress_save_load) {
    std::mt19937 rng;
    rng.seed(0);
    std::uniform_real_distribution<float> distrib_real;
    ...
    {
        auto index_hnsw = CompressMakeIndexHnsw();
        auto column_def = MakeColumnDef();
